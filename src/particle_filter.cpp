...
void Particle::updateFromObservation(
  ...
  for (const auto &p_obs : observations) {
    ...
    minus_log_weight += std::pow(p_obs.y - landmark.y_f, 2) / std_y_2;
...
void ParticleFilter::dataAssociation(const std::vector<LandmarkObs> &predicted,
  ...
  for (auto &&observation : observations) {
    ...
    for (auto &&predicted_one : predicted) {
      const auto distance = dist_squared(observation.x, observation.y,
                                         predicted_one.x, predicted_one.y);
      if (distance < min) {
        min = distance;
        observation.id = predicted_one.id;
      }
    }
  }
}
  ...
  for (auto &&p : particles_) {
    ...
    for (auto &obs : observations) {
      observations_from_particle.emplace_back(
          LandmarkObs{obs.id, obs.x * cos_theta - obs.y * sin_theta + p.x_,
                      obs.x * sin_theta + obs.y * cos_theta + p.y_});
    }

    dataAssociation(predictions, observations_from_particle);
